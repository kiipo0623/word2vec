...
class HuffmanCoding:
    ...
    def make_heap(self, frequency): #frequency has a shape of {word : frequency }
        for key in frequency: #make a node, then push in list of heap queue
            ...
            heapq.heappush(self.heap, node) # 오름차순으로 정리되는건지?
    ...
    def build(self, frequency):
        ...
        merged = self.merged_nodes() #작은 것부터 두개씩 pop하고 key=None인 새로운 노드 할당 후 자식으로 넣음
        self.make_codes() #만들어진 이진트리에서 root~each word까지 가는 길 coding(왼쪽은 0 오른쪽은 1)
